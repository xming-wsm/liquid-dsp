...
void usage() {
  ...
  printf(
      "  -n <len>  : filter length,              1 < n        default: 57\n");
  printf(
      "  -f <freq> : filter cutoff frequency,    0 < f < 0.5, default: 0.2\n");
  printf(
      "  -s <atten>: stop-band attenuation [dB], 0 < s,       default: 60\n");
}
...
int main(int argc, char *argv[]) {
  ...
  fprintf(fid,
          "title(['Filter design (firdespm) f_c: %.3f, S_L: %.3f, h: %u']);\n",
          fc, -As, n);
