...
void usage() {
  ...
  printf("  f     : filter cutoff frequency,           0 < f < 0.5, default: "
         "0.2\n");
  printf("  s     : filter stop-band attenuation [dB], 0 < s,       default: "
         "60\n");
  printf(
      "  m     : fractional sample delay,        -0.5 < m < 0.5, default: 0\n");
  printf("  n     : filter length [taps],              n > 1,       default: "
         "55\n");
}
...
int main(int argc, char *argv[]) {
  ...
  if (fc <= 0.0f) {
    fprintf(stderr,
            "error: %s, filter cutoff frequency must be greater than zero\n",
            argv[0]);
    exit(1);
  } else if (h_len == 0) {
  ...
  fprintf(fid,
          "title(['Filter design/Kaiser window f_c: %f, S_L: %f, h: %u']);\n",
          fc, -As, h_len);
