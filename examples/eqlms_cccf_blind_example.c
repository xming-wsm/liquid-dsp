...
int main(int argc, char *argv[]) {
  ...
  while ((dopt = getopt(argc, argv, "hn:s:c:k:m:b:p:u:M:")) != EOF) {
    ...
    case 'M':
      ...
      if (ms == LIQUID_MODEM_UNKNOWN) {
        fprintf(stderr,
                "error: %s, unknown/unsupported modulation scheme '%s'\n",
                argv[0], optarg);
        return 1;
      }
      break;
  ...
  if (num_symbols == 0) {
    ...
    fprintf(stderr,
            "error: %s, equalizer semi-length must be at least 1 symbol\n",
            argv[0]);
    exit(1);
  ...
  eqlms_cccf eq =
      eqlms_cccf_create_rnyquist(LIQUID_FIRFILT_RRC, k, p, beta, 0.0f);
  ...
  for (i = 0; i < num_symbols; i++) {
    fprintf(fid, "syms_rx(%4u) = %12.4e + j*%12.4e;\n", i + 1,
            crealf(syms_rx[i]), cimagf(syms_rx[i]));
  }
  ...
  fprintf(fid, "figure;\n");
  fprintf(fid,
          "plot(real(syms_rx_0),imag(syms_rx_0),'x','Color',[1 1 1]*0.7,...\n");
  fprintf(fid,
          "     real(syms_rx_1),imag(syms_rx_1),'x','Color',[1 1 1]*0.0);\n");
  ...
  fprintf(fid, "figure;\n");
  fprintf(fid, "plot(f,Hm, f,Hc, f,Hp, f,G,'-k','LineWidth',2, [-0.5/k "
               "0.5/k],[-6.026 -6.026],'or');\n");
  fprintf(fid, "xlabel('Normalized Frequency');\n");
  fprintf(fid, "ylabel('Power Spectral Density');\n");
  fprintf(fid, "legend('transmit','channel','equalizer','composite','half-"
               "power points','location','northeast');\n");
